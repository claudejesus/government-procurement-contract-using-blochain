...
//     .filter(c => {
//       // Government sees all; supplier sees only their contracts
...
//       if (activeTab === "verified") return Number(contract.status) === 2;
//       if (activeTab === "paid") return Number(contract.status) === 3;
//       if (activeTab === "rejected") return Number(contract.status) === 4;
//       return true;
//     });
...
//     if (!s) return <span className="badge bg-secondary">Unknown</span>;
...
//         {["all", "created", "completed", "verified", "paid", "rejected"].map(tab => (
//           <button
//             key={tab}
...
//                 <td>
//                   {account === c.supplier && Number(c.status) === 0 && (
//                     <button className="btn btn-warning btn-sm" onClick={() => markCompleted(c.id)}>Mark Completed</button>
//                   )}
//                   {account === government && Number(c.status) === 1 && (
//                     <button className="btn btn-success btn-sm" onClick={() => verifyContract(c.id)}>Verify</button>
//                   )}
//                   {account === government && Number(c.status) === 2 && (
//                     <button className="btn btn-info btn-sm" onClick={() => releasePayment(c.id)}>Pay</button>
//                   )}
//                   {(account === government || account === c.supplier) && Number(c.status) < 3 && (
//                     <button className="btn btn-danger btn-sm ms-1" onClick={() => cancelContract(c.id)}>Reject</button>
//                   )}
//                 </td>
//               </tr>
//             ))}
