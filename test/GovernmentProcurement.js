...
describe("GovernmentProcurement", function () {
  ...
  it("Should release payment after verification", async function () {
    await governmentProcurement.createContract(
      supplier.address,
      "Procurement 1",
      { value: ethers.parseEther("1.0") }
    ...
    await governmentProcurement.connect(supplier).markCompleted(0);
    await governmentProcurement.verifyContract(0); // ✅ Verify before payment
    ...
    // Optional: Use `gt` or `gte` to avoid exact match issues due to gas
    expect(difference).to.be.greaterThan(ethers.parseEther("0.9")); // Safe range check
  });
});
